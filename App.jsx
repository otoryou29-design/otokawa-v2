...
const [pickingDayFilter,setPickingDayFilter] = useState([]) // 複数選択: 曜日配列
const [pickingAreaFilter,setPickingAreaFilter] = useState([]) // 複数選択: エリア配列
const [eventContainerTargets,setEventContainerTargets] = useState({}) // {storeId: number}
const [deliveryDayOverrides,setDeliveryDayOverrides] = useState({}) // {area: [曜日]}
...
await initIfEmpty(“eventContainerTargets”, {})
await initIfEmpty(“deliveryDayOverrides”, {})
...
// レギュラーステータスをリセット
const emptyReg={}
setRegularStatus(emptyReg); dbSet(“regularStatus”,emptyReg)
setRegularFinalized(null); dbSet(“regularFinalized”,null)
}
...
const now=new Date()
const timeStr=now.toLocaleTimeString(“ja-JP”,{hour:“2-digit”,minute:“2-digit”})
const done2=eventProducts.filter(p=>(eventStatus[p.id]||{}).done).length
...
// 催事ステータスをリセット
const emptyStatus={}
setEventStatus(emptyStatus); dbSet(“eventStatus”,emptyStatus)
setPackageFinalized(null); dbSet(“packageFinalized”,null)
}
const resetPackage = () => {
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;500;700;900&family=IBM+Plex+Mono:wght@600;700&display=swap'); *{box-sizing:border-box;margin:0;padding:0} ::-webkit-scrollbar{width:6px}::-webkit-scrollbar-thumb{background:#b0bdb5;border-radius:4px} .card{background:#fff;border-radius:16px;box-shadow:0 2px 8px rgba(0,0,0,.06)} .fade{animation:fi .2s ease}@keyframes fi{from{opacity:0;transform:translateY(4px)}to{opacity:1}} .pulse{animation:pl 2s ease-in-out infinite}@keyframes pl{0%,100%{opacity:1}50%{opacity:.3}} .ov{position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:60;display:flex;align-items:center;justify-content:center} .modal{background:#fff;border-radius:20px;padding:28px;width:94%;max-width:640px;max-height:90vh;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,.18)} input,select,textarea{background:#f4f6f4;border:1.5px solid #dde5de;color:#1c1c1e;border-radius:10px;padding:9px 13px;font-size:14px;font-family:'Noto Sans JP',sans-serif;width:100%;outline:none} input:focus,select:focus,textarea:focus{border-color:${accent};background:#fff} .hot{font-size:11px;background:#d97706;color:#fff;padding:2px 7px;border-radius:4px;font-weight:700} ${festive?"@keyframes festive{0%,100%{background-position:0%}50%{background-position:100%}}.festive-banner{background:linear-gradient(90deg,#dc2626,#ef4444,#f97316,#ef4444,#dc2626);background-size:200%;animation:festive 3s ease infinite;color:#fff;text-align:center;padding:10px;font-weight:900;font-size:14px;letter-spacing:2px}":""}`}</style>
...
```
  ...
  <div style={{padding:"20px 18px 80px",maxWidth:1400,margin:"0 auto"}} className="fade" key={tab}>
    ...
    {tab==="stores" && (() => {
      ...
      // エリアごとの配送曜日（overrideあれば使う）
      const getDeliveryDaysForArea = (area) => {
        if(deliveryDayOverrides[area]) return deliveryDayOverrides[area]
        const days = []
        Object.entries(BASE_DELIVERY).forEach(([day,areas]) => { if(areas.includes(area)) days.push(day) })
        return days
      }
      const ALL_AREAS = [...new Set(stores.map(s=>s.area).filter(Boolean))]
      const DAY_NAMES = ["日","月","火","水","木","金","土"]
      const ALL_DAYS = ["月","火","水","木","金","土","日"]
      const todayIdx = new Date().getDay()
      const tomorrowDay = DAY_NAMES[(todayIdx+1)%7]

      // 複数選択フィルター操作
      const toggleDay = (d) => setPickingDayFilter(prev => prev.includes(d) ? prev.filter(x=>x!==d) : [...prev,d])
      const toggleArea = (a) => setPickingAreaFilter(prev => prev.includes(a) ? prev.filter(x=>x!==a) : [...prev,a])

      // フィルタリング
      const filteredStores = [...stores].filter(s => {
        const areaOk = pickingAreaFilter.length===0 || pickingAreaFilter.includes(s.area)
        if(!areaOk) return false
        if(pickingDayFilter.length===0) return true
        const storeDeliveryDays = getDeliveryDaysForArea(s.area)
        return pickingDayFilter.some(d => storeDeliveryDays.includes(d))
      }).sort((a,b) => {
      ...
      // ランク定義（日販基準）
      const RANK_DEF = [
        {rank:"S",label:"日販5万〜",color:"#6b21a8",bg:"#f3e8ff"},
        {rank:"A",label:"日販3万〜",color:"#92400e",bg:"#fef9c3"},
        {rank:"B",label:"日販2万〜",color:"#1e40af",bg:"#dbeafe"},
        {rank:"C",label:"日販1万〜",color:"#166534",bg:"#dcfce7"},
        {rank:"D",label:"1万未満", color:"#991b1b",bg:"#fee2e2"},
      ]
      ...
      <div style={{display:"grid",gap:16}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
          <div>
            ...
            <div style={{fontSize:13,color:"#6b7280",marginTop:4}}>明日の配送: <strong style={{color:accent}}>{tomorrowDay}曜日</strong></div>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            ...
            <Btn onClick={async()=>{
              ...
              const dateStr=`${now2.getMonth()+1}/${now2.getDate()}（${dayNames2[now2.getDay()]}）`
              const timeStr2=now2.toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit"})
        ...
        {/* 曜日フィルター（複数選択） */}
        <div style={{background:"#fff",borderRadius:12,padding:"12px 14px",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
          <div style={{fontSize:12,fontWeight:700,color:"#6b7280",marginBottom:8}}>📅 曜日（複数選択可・もう一度タップで解除）</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <button onClick={()=>setPickingDayFilter([])} style={{padding:"6px 13px",borderRadius:20,fontSize:13,fontWeight:pickingDayFilter.length===0?800:500,border:"2px solid",borderColor:pickingDayFilter.length===0?accent:"#dde5de",background:pickingDayFilter.length===0?accent:"#fff",color:pickingDayFilter.length===0?"#fff":"#4a5568",cursor:"pointer"}}>全曜日</button>
            {ALL_DAYS.map(d=>{
              const isActive=pickingDayFilter.includes(d)
              const isAuto=d===tomorrowDay
              return (
                <button key={d} onClick={()=>toggleDay(d)} style={{padding:"6px 13px",borderRadius:20,fontSize:13,fontWeight:isActive?800:500,border:"2px solid",borderColor:isActive?accent:"#dde5de",background:isActive?accent:isAuto?accentL:"#fff",color:isActive?"#fff":"#4a5568",cursor:"pointer",position:"relative"}}>
                  {d}{isAuto&&<span style={{position:"absolute",top:-4,right:-2,fontSize:9,background:"#d97706",color:"#fff",borderRadius:4,padding:"1px 3px"}}>明日</span>}
                </button>
              )
            })}
          </div>
        ...
        <div style={{background:"#fff",borderRadius:12,padding:"12px 14px",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div style={{fontSize:12,fontWeight:700,color:"#6b7280"}}>📍 エリア（複数選択可）</div>
            <button onClick={()=>{setShowDeliveryEdit(true);setEditDeliveryArea("");setEditDeliveryDays([])}} style={{padding:"4px 11px",fontSize:12,fontWeight:700,background:"#fff",border:"2px solid #dde5de",borderRadius:8,cursor:"pointer",color:"#4a5568"}}>✏️ 配送曜日編集</button>
          </div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <button onClick={()=>setPickingAreaFilter([])} style={{padding:"6px 13px",borderRadius:20,fontSize:13,fontWeight:pickingAreaFilter.length===0?800:500,border:"2px solid",borderColor:pickingAreaFilter.length===0?accent:"#dde5de",background:pickingAreaFilter.length===0?accent:"#fff",color:pickingAreaFilter.length===0?"#fff":"#4a5568",cursor:"pointer"}}>全エリア</button>
            {ALL_AREAS.map(a=>{
              const isActive=pickingAreaFilter.includes(a)
              const areadays=getDeliveryDaysForArea(a)
              return (
                <button key={a} onClick={()=>toggleArea(a)} style={{padding:"6px 13px",borderRadius:20,fontSize:12,fontWeight:isActive?800:500,border:"2px solid",borderColor:isActive?accent:"#dde5de",background:isActive?accent:"#fff",color:isActive?"#fff":"#4a5568",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                  <span>{a}</span>
                  <span style={{fontSize:9,opacity:.75}}>{areadays.join("・")}曜</span>
                </button>
              )
            })}
          </div>
        ...
        <div className="card" style={{padding:14}}>
          <div style={{fontSize:13,fontWeight:800,marginBottom:10}}>🏆 ランク基準（日販）</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:7}}>
            {RANK_DEF.map(r=>(
              <div key={r.rank} style={{background:r.bg,borderRadius:9,padding:"10px 6px",textAlign:"center"}}>
                <div style={{fontSize:18,fontWeight:900,color:r.color,marginBottom:3}}>{r.rank}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:11}}>
          {filteredStores.map(s=>{
            ...
            const isS=s.rank==="S", isA=s.rank==="A"
            const done=!!pickingDone[s.id]
            const containerTarget=eventContainerTargets[s.id]||""
            const storeDeliveryDays=getDeliveryDaysForArea(s.area)
            return (
              <div key={s.id} className="card" style={{padding:(isS||isA)?18:14,border:`2px solid ${done?"#16a34a":isS?"#6b21a8":isA?"#d97706":"#eef0ee"}`,background:done?"#f0fdf4":isS?"linear-gradient(135deg,#f3e8ff,#fff)":isA?"linear-gradient(135deg,#fffbeb,#fff)":"#fff",position:"relative",overflow:"hidden",opacity:done?.85:1}}>
                {isS&&!done&&<div style={{position:"absolute",top:0,right:0,width:0,height:0,borderStyle:"solid",borderWidth:"0 36px 36px 0",borderColor:"transparent #6b21a8 transparent transparent"}}/>}
                {isS&&!done&&<div style={{position:"absolute",top:4,right:3,color:"#fff",fontSize:9,fontWeight:900}}>S</div>}
                {!isS&&isA&&!done&&<div style={{position:"absolute",top:0,right:0,width:0,height:0,borderStyle:"solid",borderWidth:"0 32px 32px 0",borderColor:"transparent #d97706 transparent transparent"}}/>}
                {!isS&&isA&&!done&&<div style={{position:"absolute",top:4,right:3,color:"#fff",fontSize:9,fontWeight:900}}>A</div>}
                <div style={{marginBottom:9}}>
                  <div style={{display:"flex",gap:5,marginBottom:5,flexWrap:"wrap"}}>
                    ...
                    <span style={{fontSize:11,background:"#f0fdf4",color:"#166534",padding:"2px 7px",borderRadius:4,fontWeight:700}}>{storeDeliveryDays.join("・")}曜</span>
                    {done&&<span style={{fontSize:11,background:"#16a34a",color:"#fff",padding:"2px 8px",borderRadius:4,fontWeight:700}}>✅ 作成済み</span>}
                  </div>
                  <div style={{fontSize:(isS||isA)?16:14,fontWeight:900,color:done?"#166534":isS?"#6b21a8":isA?"#92400e":"#1c1c1e"}}>{s.name}</div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,marginBottom:9,fontSize:11,background:"#f9fafb",borderRadius:7,padding:"8px 10px"}}>
                  ...
                  <div><span style={{color:"#9ca3af"}}>催事 </span><strong>{s.eventSetup||"―"}</strong></div>
                  {s.note&&<div style={{gridColumn:"1/-1",color:"#d97706",fontWeight:700}}>⚠️ {s.note}</div>}
                </div>
                {/* 催事コンテナ目安（入力） */}
                <div style={{marginBottom:9,background:"#fef9c3",borderRadius:9,padding:"9px 12px"}}>
                  <div style={{fontSize:11,color:"#854d0e",fontWeight:700,marginBottom:6}}>📦 催事コンテナ目安</div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <input type="number" min="0" placeholder="0" value={containerTarget} onChange={e=>{const v=e.target.value;setEventContainerTargets(prev=>{const next={...prev,[s.id]:v};dbSet("eventContainerTargets",next);return next})}} style={{flex:1,padding:"6px 8px",fontSize:32,fontWeight:900,textAlign:"center",fontFamily:"'IBM Plex Mono',monospace",border:"2px solid #fde047",borderRadius:8,background:"#fff",color:"#854d0e"}}/>
                    <span style={{fontSize:16,color:"#854d0e",fontWeight:800}}>C</span>
                  </div>
                </div>
                <div style={{display:"flex",gap:7,alignItems:"flex-start",marginBottom:9}}>
                  ...
                  <div>
                    ...
                    <input type="number" min="0" placeholder="0" value={sr.caseCount||""} onChange={e=>updateShipReport(s.id,"caseCount",e.target.value)} style={{padding:"6px 8px",fontSize:15,fontWeight:800,textAlign:"center",width:72,border:`2px solid ${(isS||isA)?"#d97706":"#dde5de"}`}}/>
                  </div>
                </div>
  ...
  {/* 配送曜日編集モーダル */}
  {showDeliveryEdit && (
    <div className="ov" onClick={()=>setShowDeliveryEdit(false)}>
      <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:460}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}>
          <div style={{fontSize:17,fontWeight:900}}>✏️ エリア別 配送曜日編集</div>
          <button onClick={()=>setShowDeliveryEdit(false)} style={{background:"none",border:"none",fontSize:22,color:"#8e9e91",cursor:"pointer"}}>✕</button>
        </div>
        <div style={{marginBottom:14}}>
          <div style={{fontSize:13,fontWeight:700,marginBottom:8}}>エリアを選択</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {[...new Set(stores.map(s=>s.area).filter(Boolean))].map(a=>(
              <button key={a} onClick={()=>{
                setEditDeliveryArea(a)
                const BASE_DELIVERY={"月":["白河","須賀川","郡山","矢吹"],"火":["会津","福島"],"水":["白河","須賀川","郡山","矢吹"],"木":["いわき","福島"],"金":["会津"],"土":["白河","須賀川","郡山","矢吹","いわき"],"日":["会津","福島"]}
                if(deliveryDayOverrides[a]){setEditDeliveryDays(deliveryDayOverrides[a])}
                else{const days=[];Object.entries(BASE_DELIVERY).forEach(([d,areas])=>{if(areas.includes(a))days.push(d)});setEditDeliveryDays(days)}
              }} style={{padding:"7px 14px",borderRadius:20,fontSize:13,fontWeight:editDeliveryArea===a?800:500,border:"2px solid",borderColor:editDeliveryArea===a?accent:"#dde5de",background:editDeliveryArea===a?accent:"#fff",color:editDeliveryArea===a?"#fff":"#4a5568",cursor:"pointer"}}>{a}</button>
            ))}
          </div>
        </div>
        {editDeliveryArea&&(
          <div style={{marginBottom:16}}>
            <div style={{fontSize:13,fontWeight:700,marginBottom:8}}>{editDeliveryArea}の配送曜日（複数選択）</div>
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              {["月","火","水","木","金","土","日"].map(d=>{
                const isActive=editDeliveryDays.includes(d)
                return (
                  <button key={d} onClick={()=>setEditDeliveryDays(prev=>isActive?prev.filter(x=>x!==d):[...prev,d])} style={{padding:"8px 16px",borderRadius:20,fontSize:14,fontWeight:isActive?800:500,border:"2px solid",borderColor:isActive?accent:"#dde5de",background:isActive?accent:"#fff",color:isActive?"#fff":"#4a5568",cursor:"pointer"}}>{d}曜</button>
                )
              })}
            </div>
            <div style={{marginTop:9,fontSize:13,color:"#6b7280"}}>選択中: <strong style={{color:accent}}>{editDeliveryDays.join("・")}曜日</strong></div>
          </div>
        )}
        <div style={{display:"flex",gap:9,justifyContent:"flex-end",marginTop:8}}>
          <Btn outline color={accent} onClick={()=>setShowDeliveryEdit(false)} style={{padding:"8px 16px"}}>キャンセル</Btn>
          <Btn onClick={()=>{
            if(!editDeliveryArea) return
            const next={...deliveryDayOverrides,[editDeliveryArea]:editDeliveryDays}
            setDeliveryDayOverrides(next); dbSet("deliveryDayOverrides",next); setShowDeliveryEdit(false)
          }} style={{padding:"8px 22px",background:editDeliveryArea?accent:"#d1d5db",fontSize:14}}>✅ 保存する</Btn>
        </div>
      </div>
    </div>
  )}
